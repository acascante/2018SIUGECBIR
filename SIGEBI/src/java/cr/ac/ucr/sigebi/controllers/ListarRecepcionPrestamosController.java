...
public class ListarRecepcionPrestamosController extends BaseController {
    ...
    private void contarPrestamos() {
        try {
            Long contador = recepcionPrestamoModel.contar(command.getFltIdCodigo(), command.getFltConvenio(), command.getFltId(), command.getFltDescripcion(), command.getFltFechaIngreso(), command.getFltFechaDevolucion(), command.getFltEstado());
    ...
    private void listarPrestamos() {
        try {
            this.prestamos = recepcionPrestamoModel.listar(this.getPrimerRegistro()-1, this.getUltimoRegistro(), command.getFltIdCodigo(), command.getFltConvenio(), command.getFltId(), command.getFltDescripcion(), command.getFltFechaIngreso(), command.getFltFechaDevolucion(), command.getFltEstado());
        } catch (FWExcepcion e) {
            Mensaje.agregarErrorAdvertencia(e.getError_para_usuario());
        }
    }
